...
The command is idempotent for identical inputs and refuses differing files, symlinks, and unsafe language paths. It records the six package-managed runtime files in `.unaltraweb/scaffold.json`; an explicit `scaffold_sync` dry-run can later update only files that remain unchanged from that baseline. It does not require or inspect `unaltraweb-template`.
