...
- **Deployment:** GitHub Pages through local `make publish` to `gh-pages` by default, or manual GitHub Actions when local publishing is not possible
- **File Structure:**
...
# Publish locally with make publish, or run the manual GitHub Actions deploy workflow
```
...
6. **Only then push to GitHub** – Once everything looks good locally, commit and push. Publishing is explicit and does not happen automatically on every push:
   ```bash
   git add .
...
**Why this matters:** Catching errors locally saves time, avoids unnecessary GitHub Actions runs, and prevents broken content from going live. Most issues are easy to spot in the local preview.
...
- **Expecting every push to deploy** – Deploy workflows are manual by default. Publish locally with `make publish`, or explicitly run the deploy workflow from the **Actions** tab.
- **Modifying the `gh-pages` branch directly** – Never edit this branch. It's generated by local publishing or the manual deploy workflow. All source changes go to `main`.
...
- 🚫 **Never do:**
  ...
  - Edit generated files in `_site/` or the `gh-pages` branch
...
- The `gh-pages` branch is generated by `make publish` or an explicit manual deploy workflow
- Pushing to `main` does not publish by itself; run `make publish` or the manual deploy workflow when the site should go live
...
- After making changes, clearly explain how to preview locally and how to publish explicitly with `make publish` or the manual deploy workflow
