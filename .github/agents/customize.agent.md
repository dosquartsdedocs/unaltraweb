...
- **Deployment:** GitHub Pages through the generated manual GitHub Actions workflow
- **File Structure:**
...
# Publish by explicitly running the manual GitHub Actions deploy workflow
```
...
- **Expecting every push to deploy** – Deploy workflows are manual by default. Explicitly run the deploy workflow from the **Actions** tab.
- **Modifying generated Pages output directly** – All source changes belong on `main`; GitHub Pages deployment artefacts are generated by the manual workflow.
...
- All changes should be made to the **main** (or **source**) branch, never to generated Pages output
- Pushing to `main` does not publish by itself; run the manual deploy workflow when the site should go live
...
- After making changes, clearly explain how to preview locally and how to publish explicitly with the manual deploy workflow
