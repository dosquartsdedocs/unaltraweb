...
The default template is a two-sided, open-right book: inner and outer margins mirror, folios sit at the outer edge, numbered and unnumbered chapters begin on recto pages, and inserted blank versos carry no headers or folios. Main matter starts at chapter 0. Fenced code uses the same visible language contract as the web: a localized generic label for plain or unlabelled code, a canonical language name for highlighted code, line numbers, and breakable long lines inside a page-aware panel.
