...
The default template is a two-sided, open-right book: inner and outer margins mirror, folios sit at the outer edge, numbered and unnumbered chapters begin on recto pages, and inserted blank versos carry no headers or folios. Main matter starts at chapter 0, and each chapter opens as a single `number. title` heading. The secondary cover color is shared by complete headings and figure or table labels. Fenced code uses the same visible language contract as the web: a localized generic label for plain or unlabelled code, a canonical language name, line numbers, and breakable long lines inside a page-aware `listings` panel. PDF syntax highlighting is guaranteed for Bash, SQL, Python, and R; unsupported identifiers retain their label and use the plain-text panel.
