...
- Package scaffold changes affect newly generated sites. Existing generated repositories can review `scaffold_sync`, which manages only `.gitignore`, `.unaltraweb/docker-mount.sh`, Makefile, Gemfile/lock, and the deploy workflow from `.unaltraweb/scaffold.json`; it never overwrites conflicts or changes config/content.
